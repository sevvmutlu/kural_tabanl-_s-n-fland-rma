...
df = pd.read_csv("C:/Users/USER/Desktop/python_temelleri/persona.csv")
df.head()
df.info()
df.shape
df.tail()
...
# Veri setine ekleyelim ve gereksiz değişkenleri çıkaralım:
...
# Ufak bir problemimiz var. Bir çok aynı segment olacak.
...
agg_df1.groupby('SEGMENT').agg({'PRICE': ['mean', 'min', 'max', 'sum', 'count', ]}).sort_values('SEGMENT', ascending=False)
